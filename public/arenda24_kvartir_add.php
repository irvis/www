...
<html>
<head>
  <title>Подать объявление о сдаче в аренду посуточно квартиры в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
<div class="content_box">
  ...
  <div class="content_header">
        ...
        <b>Аренда квартир посуточно в Великом Новгороде и Новгородской области</b>
        </div>
  </div>
...
if ($glogged==1) { // Пользователь авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    $errorlist="";
    // здесь вставляется проверка параметров (добавление квартиры)
    require "./_parts/check_arenda24_kvartir.php";
    ...
    // Если ошибок нет
      ...
      $query="INSERT INTO rent24_kva SET ";
      //$query.="status='0',";
      //$query.="checked_fl='0',";
      ...
      unilog_add_record (
		 ...
		 "13",  // Тип объекта - Квартира в аренду посуточно - 13
		 $last_id, // Номер объекта
		 "Добавление объявления: ".get_notice_by_object ($db, 13, $last_id),   // Описание - генерация текста объявления
		 "", // "Описание: ".get_kv_rent_descr_by_id ($db, $last_id), // Допинфа1 - Описание
		 "" // "Контакты: ".get_kv_rent_conta_by_id ($db, $last_id)  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
	  
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление добавлено!</b> Посмотреть опубликованное объявление можно <a href=\"./sdam24_kvartiru.php?id=".$last_id."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
...
?>    
       ...
       <h2>Сдать <b class="red">посуточно</b> квартиру в Великом Новгороде</h2>
       <div class="clear"></div>
       
       <form name="myform" action="arenda24_kvartir_add.php" method="post">
       <!-- ТАБЛИЦА -->
       <table width="690" cellpadding="5" cellspacing="0">
       <tr>
         <td>Количество комнат * :</td>
         <td>
           <input name="kva_rooms" type="edit" size="3" value="<?php if (isset($_POST['kva_rooms'])) { echo $_POST['kva_rooms']; };?>">
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="kva_pl_ob" size="4" type="edit" value="<?php if (isset($_POST['kva_pl_ob'])) { echo $_POST['kva_pl_ob']; };?>">
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="kva_pl_zh" size="4" type="edit" value="<?php if (isset($_POST['kva_pl_zh'])) { echo $_POST['kva_pl_zh']; };?>">
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="kva_pl_kuh" size="4" type="edit" value="<?php if (isset($_POST['kva_pl_kuh'])) { echo $_POST['kva_pl_kuh']; };?>">
         </td>
       </tr>
       ...
       <tr>
         <td>Этаж * :</td>
         <td>
           <input name="kva_floor" type="edit" size="3" value="<?php if (isset($_POST['kva_floor'])) { echo $_POST['kva_floor']; };?>">
         </td>
       </tr>
       ...
       <tr>
         <td>Балкон / лоджия:</td>
         <td>
           <input name="kva_balkon" value="1" type="checkbox" <?php if (isset($_POST['kva_balkon'])) { echo "checked"; };?> > есть балкон / лоджия
         </td>
       </tr>
       
       <tr>
         <td>Стоимость <b class="red">в сутки</b> (руб.) * :</td>
         <td>
       ...
       <tr>
         <td>Примечание:</td>
         <td>
           <textarea title="Дополнительная информация по квартире (Например: сделан евроремонт, установлены стеклопакеты)" name="descr" cols="30" rows="4"><?php if (isset($_POST['descr'])) { echo $_POST['descr']; }; ?></textarea>
         </td>
       </tr>
