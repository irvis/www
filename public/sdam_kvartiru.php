...
if (isset($_GET['id'])) {
  $kolvo=0;
  $query="SELECT * FROM rent_kva WHERE status='0' AND id='".(int)($_GET['id'])."'";
  $res=$db->query($query); 
  $kolvo=$res->num_rows;
  if ($kolvo!=0) {
...
// ======================================================================
  // Формируем заголовки
  $super_title="";
  $super_title.="Сдам ".$ar[kva_rooms]."-комн. квартиру ";
...
<div class="content_box">
  ...
  <div class="content_cover">
    <div class="content_inner" style="width:700px;">
      ...
      <div class="l_date">
          <a href="./arenda_kvartir_v_velikom_novgorode.php" class="link"><img width="16" height="16" align="absmiddle" src="./pics/ico_uplist.png" border="0"></a> 
          <a href="./arenda_kvartir_v_velikom_novgorode.php" class="link">Назад к списку</a>
          
          <?php if (isset($_SESSION['id']) && $ar['usr_id']==$_SESSION['id']) { ?>
             &nbsp;&nbsp;&nbsp;
            <a href="./arenda_kvartir_edit.php?id=<?php echo $ar['id']; ?>" class="link"><img width="16" height="16" align="absmiddle" src="./pics/ico_edit.png"></a> 
            <a href="./arenda_kvartir_edit.php?id=<?php echo $ar['id']; ?>" class="link">Редактировать</a>
            &nbsp;&nbsp;&nbsp;
            <a href="./arenda_kvartir_delete.php?id=<?php echo $ar['id']; ?>" class="link" onclick="return confirmDelete();"><img width="16" height="16" align="absmiddle" src="./pics/ico_delete.png"></a> 
            <a href="./arenda_kvartir_delete.php?id=<?php echo $ar['id']; ?>" class="link" onclick="return confirmDelete();">Удалить</a>
            &nbsp;&nbsp;&nbsp;
            <a href="./arenda_kvartir_up.php?id=<?php echo $ar['id']; ?>" class="link"><img width="16" height="16" align="absmiddle" src="./pics/ico_up.png"></a> 
            <a href="./arenda_kvartir_up.php?id=<?php echo $ar['id']; ?>" class="link">Поднять</a>
            &nbsp;&nbsp;&nbsp;
      ...
      <div class="part_right340" style="border:0px solid red;">
           ...
           if (strlen($ar['geo_yamap'])==0) {
             ...
             $query="UPDATE sell_kmn 
                      SET geo_yamap='".$db->escape_string($yamap_coord)."'
                      WHERE id='".(int)($_GET['id'])."';";
             $res=$db->query($query); //echo "<b style=\"color:red;\">ZAPROS=[".$query."]</b>"; //debug
...
<?php
         ...
         // ==========================================================================================================
         // === УНИВЕРСАЛЬНЫЙ ДВИЖОК ДЛЯ ФОТОГРАФИЙ (начало) =========================================================
         // ==========================================================================================================  
         
         // НАСТРОЙКА ПАРАМЕТРОВ 
         $obj_type='12';             // Задаем тип: аренда квартиры
         $obj_id=(int)($_GET['id']); // Задаем номер объекта
         $notice=get_notice_by_object ($db, $obj_type, $obj_id); // Получаем текст объявления - для подписывания фото
         
         $avtor=false; if ($ar['usr_id']==$_SESSION['id']) { $avtor=true; }; // Флажок авторства
         $admin=false; if (check_admin($_SESSION['id']))   { $admin=true; }; // Флажок админства
         
         require "./_parts/_photo_engine.php"; 
         
         // ==========================================================================================================
         // === УНИВЕРСАЛЬНЫЙ ДВИЖОК ДЛЯ ФОТОГРАФИЙ (конец) ==========================================================
         // ==========================================================================================================  
...
<?php
	echo "<div class=\"clear\"></div>";
       ...
       ?>
         ...
         <div class="clear" style="margin-top:20px;margin-bottom:20px;border-top:4px solid red;"></div>
         
         Автор: <?php echo get_user_info($db,$ar['usr_id']); ?> <br>
         IP-адрес: <?php echo $ar['usr_ip']; ?>
