...
<html>
<head>
  <title>Редактирование объявления о продаже комнаты в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
<div class="content_box">
  ...
  <div class="content_header">
        ...
        <b>Продажа комнат в Великом Новгороде и Новгородской области</b>
        </div>
  </div>
...
if (!($glogged==1)) { // Пользователь не авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    $errorlist="";
    // проверка параметров комнаты (отправка формы)
    require "./_parts/check_komnata.php";
    ...
    if ($errorlist!="") {
      ...
      $query.="geo_rayon_id='".$db->escape_string($_POST['geo_rayon_id'])."',";
      if (($_POST['geo_rayon_id']=="-1") && (strlen(trim($_POST['geo_rayon']))>0) ) {
        $query.="geo_rayon='".$db->escape_string($_POST['geo_rayon'])."',";
      };
      
      $query.="geo_street_id='".$db->escape_string($_POST['geo_street_id'])."',";
      if (($_POST['geo_street_id']=="-1") && (strlen(trim($_POST['geo_street']))>0) ) {
        $query.="geo_street='".$db->escape_string($_POST['geo_street'])."',";
      };
      
      if (strlen(trim($_POST['geo_n_doma'])>0) ) {
        $query.="geo_n_doma='".$db->escape_string($_POST['geo_n_doma'])."',";
        $query.="geo_n_korp='".$db->escape_string($_POST['geo_n_korp'])."',";
      };
      
      
      $query.="zd_floors='".$db->escape_string($_POST['zd_floors'])."',";
      $query.="zd_material_id='".$db->escape_string($_POST['zd_material_id'])."',";
      
      
      // Для комнаты
      // +++++++++++++++++++++++++
      $query.="kmn_type_id='".$db->escape_string($_POST['kmn_type_id'])."',";
      $query.="kmn_floor='".$db->escape_string($_POST['kmn_floor'])."',";
      $query.="kmn_pl='".$db->escape_string(str_replace(",", ".", $_POST['kmn_pl']))."',";
      $query.="kmn_balkon='".$db->escape_string($_POST['kmn_balkon'])."',";
      $query.="geo_yamap='',";
      // +++++++++++++++++++++++++
      ...
      $query.="WHERE id='".(int)$_GET['id']."' AND usr_id='".$_SESSION['id']."';";
      
      // Записываем в журнал СТАРЫЕ ДАННЫЕ
      loging_add_kmnrecord ($db, $_SESSION['id'], "203", (int)$_GET['id'], get_kmn_obyav_by_id($db,(int)$_GET['id']), get_kmn_descr_by_id ($db, (int)$_GET['id']), get_kmn_conta_by_id ($db, (int)$_GET['id']) );
      ...
      unilog_add_record (
		 ...
		 "Редактирование (старое): ".get_kmn_obyav_by_id($db,(int)$_GET['id']),   // Описание - генерация текста объявления
		 "Старое описание: ".get_kmn_descr_by_id ($db, (int)$_GET['id']), // Допинфа1 - Описание
		 "Старое контакты: ".get_kmn_conta_by_id ($db, (int)$_GET['id'])  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
      ...
      unilog_add_record (
		 ...
		 "21",  // Тип объекта - Комната на продажу - 21
		 (int)$_GET['id'], // Номер объекта
		 "Редактирование (новое): ".get_kmn_obyav_by_id($db,(int)$_GET['id']),   // Описание - генерация текста объявления
		 "Новое описание: ".get_kmn_descr_by_id ($db, (int)$_GET['id']), // Допинфа1 - Описание
		 "Новые контакты: ".get_kmn_conta_by_id ($db, (int)$_GET['id'])  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
	  
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление успешно отредактировано!</b> Посмотреть отредактированное объявление можно <a href=\"./prodam_komnatu.php?id=".(int)$_GET['id']."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
  ...
  if (($found) && ($showme)) {  ?>    
       <h2>Редактирование объявления о продаже комнаты</h2>
       <div class="clear"></div>
       
       <form name="myform" action="komnata_edit.php?id=<?php echo intval($_GET['id']); ?>" method="post">
       <!-- ТАБЛИЦА -->
       ...
       <tr>
         ...
         <td>
           <select name="kmn_type_id">
             <?php printselectsortdef($db, 'spr_kmn_type', 'sort', 'ASC', $ar['kmn_type_id']); ?>
           </select>
         </td>  
       </tr>
       ...
       <tr>
         <td>Этаж * :</td>
         <td>
           <input name="kmn_floor" type="edit" size="3" value="<?php if (isset($ar['kmn_floor'])) { echo $ar['kmn_floor']; };?>">
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="kmn_pl" size="4" type="edit" value="<?php if (isset($ar['kmn_pl'])) { echo $ar['kmn_pl']; };?>">
         </td>
       </tr>
       ...
       <tr>
         <td>Балкон / лоджия:</td>
         <td>
           <input name="kmn_balkon" value="1" type="checkbox" <?php if ((isset($ar['kmn_balkon'])) && ($ar['kmn_balkon']=='1') ) { echo "checked"; };?> > есть балкон / лоджия
         </td>
       </tr>
