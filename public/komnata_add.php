...
<html>
<head>
  <title>Подать объявление о продаже комнаты в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
if ($glogged==1) { // Пользователь авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    // Если ошибок нет
      ...
      $query="INSERT INTO sell_kmn SET ";
      //$query.="status='0',";
      //$query.="checked_fl='0',";
      ...
      // Записываем в журнал
      loging_add_kmnrecord ($db, $_SESSION['id'], "201", $last_id, get_kmn_obyav_by_id($db,$last_id), get_kmn_descr_by_id ($db, $last_id), get_kmn_conta_by_id ($db, $last_id) );
      ...
      unilog_add_record (
		 ...
		 "Добавление объявления: ".get_kmn_obyav_by_id($db,$last_id),   // Описание - генерация текста объявления
		 "Описание: ".get_kmn_descr_by_id ($db, $last_id), // Допинфа1 - Описание
		 "Контакты: ".get_kmn_conta_by_id ($db, $last_id)  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
	  
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление добавлено!</b> Посмотреть опубликованное объявление можно <a href=\"./prodam_komnatu.php?id=".$last_id."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
...
?>    
       ...
       <h2>Продать комнату в Великом Новгороде</h2>
       <div class="clear"></div>
       
       <form name="myform" action="komnata_add.php" method="post">
       <!-- ТАБЛИЦА -->
