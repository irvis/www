...
function check_admin($s_item) {
  ...
  $array = array(1 => 1018);   // это 5552850 , а если два и более - $array = array(1 => 1018, 2 => 1);
...
function get_user_info($link, $uid) {
  ...
  $res=mysql_query($zapros,$link); //$kolvo=mysql_num_rows($res);
  if (($ar=mysql_fetch_assoc($res)) == true ) {
     $s='';
     $s.='id='.$ar['id'].', ';
     $s.=$ar['name'];
     $s.=' ('.$ar['nickname'].'), ';
...
function printselectdef($link, $tname, $def) {
  ...
  while (($ar=mysql_fetch_assoc($res)) == true ) {
    if ($ar['id']==$def) {
      echo "<option value=".$ar['id']." selected>".$ar['title']."</option>\n";
    } else {
      echo "<option value=".$ar['id'].">".$ar['title']."</option>\n";
    };
  };
...
function printselectsort($link, $tname, $sort, $sort_type) {
  $zapros="SELECT * FROM ".$tname." ORDER BY ".$sort." ".$sort_type.", title ASC";
  $res=mysql_query($zapros,$link); //$kolvo=mysql_num_rows($res);
  while (($ar=mysql_fetch_assoc($res)) == true ) {
    echo "<option value=".$ar['id'].">".$ar['title']."</option>\n";
  };
...
function printselectsort_title_d($link, $tname, $def) {
  ...
  while (($ar=mysql_fetch_assoc($res)) == true ) {
    if ($ar['id']==$def) {
      echo "<option value=".$ar['id']." selected>".$ar['title_d']."</option>\n";
    } else {
      echo "<option value=".$ar['id'].">".$ar['title_d']."</option>\n";
    };
  };
};
...
function exist_db_email ($db , $email) {
  $sql = "SELECT * FROM tbl_users WHERE email='".mysql_real_escape_string($email)."' LIMIT 1;";
...
function exist_db_phone ($db , $phone) {
  $sql = "SELECT * FROM tbl_users WHERE phone='".mysql_real_escape_string($phone)."' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  if ($result)  {  
    // если существует запись то возвращаем истину иначе ложь   
    if (mysql_num_rows($result) == 1) {    
...
function exist_db_agent ($db , $user_id) {
  $sql = "SELECT * FROM spr_realty_agent WHERE id='".mysql_real_escape_string($user_id)."' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  if ($result)  {  
    // если существует запись то возвращаем истину иначе ложь   
    if (mysql_num_rows($result) == 1) {    
...
function active_db_id ($db , $id) {
  $sql = "SELECT * FROM tbl_users WHERE id='".(int)($id)."' AND status='1' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  if ($result)  {  
    // если существует запись то возвращаем истину иначе ложь   
    if (mysql_num_rows($result) == 1) {    
...
function owner_of ($tbl, $db , $user, $id) {
  $sql = "SELECT * FROM ".$tbl." WHERE id='".(int)($id)."' AND usr_id='".(int)($user)."' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  if ($result)  {  
    // если существует запись то возвращаем истину иначе ложь   
    if (mysql_num_rows($result) == 1) {    
...
function mprotected($email) {
  ...
  echo "<script language=\"JavaScript\">
        ...
        var server=\"".$str2."\";
        var domain=\"".$str3."\";
        document.write('<a class=\"link\" href=\"' + 'mail' + 'to:' + name + '@' + server + '.' + domain + '\">'
        + name + '@' + server + '.' + domain + '</a>');
        </script>
        <noscript>
          <b style=\"color:red;\">Включите поддержку JavaScript</b>
        </noscript>";
...
function my_counter_tbl ($tbl, $db, $user_id) {
  $sql = "SELECT COUNT(*) FROM ".$tbl." WHERE status='0' AND usr_id='".$user_id."' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
...
function counter_tbl_usl ($tbl, $db, $usl) {
  $sql = "SELECT COUNT(*) FROM ".$tbl." WHERE status='0' AND ".$usl." LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  $row = mysql_fetch_row($result);
  if ($result)  {  
    return $row[0];    
...
function count_typed ($db , $type_id) {
  $sql = "SELECT COUNT(*) FROM tbl_objects WHERE status='0' AND obj_type_id='".mysql_real_escape_string($type_id)."' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  $row = mysql_fetch_row($result);
  if ($result)  {  
    return $row[0];    
...
// Возвращает количество записей в таблице tbl_objects с данным type_id + kv_rooms
function count_typed_roomed ($db , $type_id, $kv_rooms) {
  $sql = "SELECT COUNT(*) FROM tbl_objects WHERE status='0' AND obj_type_id='".mysql_real_escape_string($type_id)."' AND kv_rooms='".mysql_real_escape_string($kv_rooms)."' LIMIT 1;";
  if ($kv_rooms>=4) {
    $sql = "SELECT COUNT(*) FROM tbl_objects WHERE status='0' AND obj_type_id='".mysql_real_escape_string($type_id)."' AND kv_rooms>='4' LIMIT 1;";
  };
  $result = mysql_query($sql,$db) or die(mysql_error());
  $row = mysql_fetch_row($result);
  if ($result)  {  
...
function count_objects ($db , $user_id) {
  $sql = "SELECT COUNT(*) FROM tbl_objects WHERE status='0' AND usr_id='".mysql_real_escape_string($user_id)."' LIMIT 1;";
  $result = mysql_query($sql,$db) or die(mysql_error());
  $row = mysql_fetch_row($result);
  if ($result)  {  
    return $row[0];    
...
// Функция для логирования действий с квартирами
          function loging_add_kvrecord ($db, $p_user_id, $p_action_id, $p_kv_id, $p_descr, $p_xtra1, $p_xtra2) {
...
function get_punkt_by_id ($db, $punkt_id) {
  $query="SELECT * 
          FROM spr_geo_gorod 
          WHERE id='".(int)$punkt_id."'
          ";
  $res=mysql_query($query,$db);
  $punkt_kolvo=mysql_num_rows($res);
...
function get_street_by_id ($db, $street_id) {
  $query="SELECT * 
          FROM spr_geo_street 
          WHERE id='".(int)$street_id."'
          ";
  $res=mysql_query($query,$db);
  $street_kolvo=mysql_num_rows($res);
  $ar=mysql_fetch_assoc($res);
  if ($street_kolvo=='1'  ) {
    // Есть такая улица
    return $ar['title_d'];
...
function get_material_by_id ($db, $material_id) {
  $query="SELECT * 
          FROM spr_zd_material  
          WHERE id='".(int)$material_id."'
          ";
  $res=mysql_query($query,$db);
  $material_kolvo=mysql_num_rows($res);
  $ar=mysql_fetch_assoc($res);
  if ($material_kolvo=='1'  ) {
    // Есть такой материал
    return $ar['title_d'];
...
// ================================================================================
// Квартира на продажу (начало)====================================================
  if ($obj_type=='11') { 

	$price_ed="тыс.руб.";
	
    $query="SELECT * FROM sell_kva WHERE id='".$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
	$kolvo='0';
  ...
  if ($kolvo=='1'  ) {  // Есть такое объявление
	...
	$ld.=$ar['kva_pl_ob']."/".$ar['kva_pl_zh']."/".$ar['kva_pl_kuh'].", ";
	$ld.=$ar['kva_floor']."/".$ar['zd_floors'].get_material_by_id($db,$ar['zd_material_id']).", ";
    
    // + ГОРОД
    if ($ar['geo_gorod_id']!="1")   {  // Великий Новгород не пишем (это по умолчанию)
    if ($ar['geo_gorod_id']!="-1")  {  // если есть номер города, тогда извлекаем название
      $ld.=get_punkt_by_id($db,$ar['geo_gorod_id']).", "; 
    } else {  // если нет номера города (номер =-1)
      if (strlen(trim($ar['geo_gorod']))>0) { // город указан вручную
        $ld.=$ar['geo_gorod'].", ";
      } else { // город НЕ указан вручную, невозможная ситуация - т.к. должен быть указан город
    ...
    // + УЛИЦА
    if ($ar['geo_street_id']!="-1")  {  // если есть номер улицы, тогда извлекаем название
      $ld.=get_street_by_id($db,$ar['geo_street_id']).", "; 
    } else {  // если нет номера улицы (номер =-1)
      if (strlen(trim($ar['geo_street']))>0) {  // улица указана вручную
        $ld.=$ar['geo_street'].", ";
      } else {  // улица НЕ указана вручную, невозможная ситуация - т.к. должна быть указана улица
    ...
    // + КОРПУС ДОМА
    if (strlen($ar['geo_n_korp'])>0) { $ld.="к. ".$ar['geo_n_korp']." "; }; 
    ...
    return "[Ошибка: неизвестное объявление о продаже квартиры, id=".$obj_id."]";
  };
	
	
  }; 
// Квартира на продажу (конец)=====================================================
// ================================================================================
...
// ================================================================================
// Квартира в аренду (начало)======================================================
  if ($obj_type=='12') { 
    
	$price_ed="руб./мес.";
	
    $query="SELECT * FROM rent_kva WHERE id='".$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
	$kolvo='0';
    $kolvo=mysql_num_rows($res);
  if ($kolvo=='1'  ) {  // Есть такое объявление
    ...
    // Нет такого объявления
    return "[Ошибка: неизвестное объявление об аренде квартиры, id=".$obj_id."]";
  };
	
	
  }; 
// Квартира в аренду (конец)=====================================================
// ==============================================================================


  
// ================================================================================
// Квартира в аренду посуточно (начало)============================================
  if ($obj_type=='13') { 
  
  $price_ed="руб./сутки";
	
    $query="SELECT * FROM rent24_kva WHERE id='".$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
	$kolvo='0';
    $kolvo=mysql_num_rows($res);
  if ($kolvo=='1'  ) {  // Есть такое объявление
    ...
    // Нет такого объявления
    return "[Ошибка: неизвестное объявление о посуточной аренде квартиры, id=".$obj_id."]";
  };
  
  }; 
// Квартира в аренду посуточно (конец)=============================================
// ================================================================================
  
  

// ================================================================================
// Комната на продажу (начало)=====================================================
  if ($obj_type=='21') { 
    $price_ed="тыс. руб.";
    $query="SELECT * FROM sell_kmn WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      $ar=mysql_fetch_assoc($res);
      $ld.="Продам ";
      $ld.=get_kmntype_by_id($db,$ar['kmn_type_id'])." пл. ".$ar['kmn_pl']."кв.м, ".$ar['kmn_floor']."/".$ar['zd_floors'].get_material_by_id($db,$ar['zd_material_id']).", ";
      ...
      if ($ar['geo_gorod_id']!="-1")  {
        ...
        if (strlen(trim($ar['geo_gorod']))>0) {
          ...
          // город НЕ указан вручную, невозможная ситуация - т.к. должен быть указан город
      ...
      if ($ar['geo_street_id']!="-1")  {
        ...
        if (strlen(trim($ar['geo_street']))>0) {
          ...
          // улица НЕ указана вручную, невозможная ситуация - т.к. должна быть указана улица
      ...
      if ($ar['price_torg']=='5') {
          // без торга
          $ld.="за ".(int)$ar['price']." ".$price_ed.", без торга";
        } else {
          if ($ar['price_torg']=='4') { 
            // возможен торг
            $ld.="за ".(int)$ar['price']." ".$price_ed.", торг";
          } else {     
            // не указано про торг
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление о продаже комнаты, id=".(int)$obj_id."]";
    };

  
  }; 
// Комната на продажу (конец)=====================================================
// ===============================================================================
...
// ================================================================================
// Комната в аренду (начало)=======================================================
  if ($obj_type=='22') {
    
    $price_ed="руб./мес.";
    $query="SELECT * FROM rent_kmn WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление об аренде комнаты, id=".(int)$obj_id."]";
    };
 
  }; 
// Комната в аренду (конец)=======================================================
// ===============================================================================
  
  
  
// ================================================================================
// Комната в аренду посуточно (начало)=============================================
  if ($obj_type=='23') { 
    
    $price_ed="руб./сутки";
    $query="SELECT * FROM rent24_kmn WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление о посуточной аренде комнаты, id=".(int)$obj_id."]";
    };
    
    
    
  }; 
// Комната в аренду посуточно (конец)=============================================
// ===============================================================================
...
// ================================================================================
// Дом, коттедж на продажу (начало)================================================
  if ($obj_type=='51') { 
    $price_ed="тыс. руб.";
    $query="SELECT * FROM sell_dom WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      $ar=mysql_fetch_assoc($res);
      $ld.="Продам ";
      $ld.=get_domtype_by_id($db,$ar['dom_type'])." (".get_dommaterial_by_id($db,$ar['dom_material_id'])."), ";
      
      $ld.="пл. ".$ar['dom_pl_ob'];
      if ($ar['dom_pl_zh']>0) { $ld.="/".$ar['dom_pl_zh']; };
      $ld.=" кв.м, ";
      
      if ($ar['dom_floors']>=2) { $ld.=$ar['dom_floors']."-эт., "; };
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление о продаже дома/коттеджа, id=".(int)$obj_id."]";
    };

  
  }; 
// Дом, коттедж на продажу (конец)================================================
// ===============================================================================
...
// =================================================================================
// Дом, коттедж в аренду (начало)===================================================
  if ($obj_type=='52') { 
    $price_ed="руб./мес.";
    $query="SELECT * FROM rent_dom WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление об аренде дома/коттеджа, id=".(int)$obj_id."]";
    };
  
  
  
  }; 
// Дом, коттедж в аренду (конец)===================================================
// ================================================================================
...
// =================================================================================
// Дом, коттедж в аренду посуточно (начало)=========================================
  if ($obj_type=='53') { 
    $price_ed="руб./сутки";
    $query="SELECT * FROM rent24_dom WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление о посуточной аренде дома/коттеджа, id=".(int)$obj_id."]";
    };
    
    
  }; 
// Дом, коттедж в аренду посуточно (конец)=========================================
// ================================================================================
...
// =================================================================================
// Участок, дача на продажу (начало)================================================
  if ($obj_type=='71') { 
    $price_ed="тыс. руб.";
    $query="SELECT * FROM sell_uch WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление о продаже участка/дачи, id=".(int)$obj_id."]";
    };

  
  }; 
// Участок, дача на продажу (конец)================================================
// ================================================================================
...
// =================================================================================
// Участок, дача в аренду (начало)==================================================
  if ($obj_type=='72') { 
    $price_ed="руб./мес.";
    $query="SELECT * FROM rent_uch WHERE id='".(int)$obj_id."' AND status='0'";
    $res=mysql_query($query,$db);
    $kolvo=mysql_num_rows($res);
    if ($kolvo=='1'  ) {
      // Есть такое объявление
      ...
      // Нет такого объявления
      return "[Ошибка: неизвестное объявление об аренде участка/дачи, id=".(int)$obj_id."]";
    };
    
    
  }; 
// Участок, дача в аренду (конец)==================================================
// ================================================================================
...
function get_kv_obyav_by_id ($db, $kv_id) {
  ...
  if ($kv_kolvo=='1'  ) {
    ...
    $ld="";
    $ld.="Продам ";
    $ld.=$ar['kva_rooms']."-к.кв., ".$ar['kva_floor']."/".$ar['zd_floors'].get_material_by_id($db,$ar['zd_material_id']).", ".$ar['kva_pl_ob']."/".$ar['kva_pl_zh']."/".$ar['kva_pl_kuh'].", ";
...
// Функция для логирования действий с комнатами
          function loging_add_kmnrecord ($db, $p_user_id, $p_action_id, $p_kmn_id, $p_descr, $p_xtra1, $p_xtra2) {
            $query="INSERT INTO logs 
                        (id,
                         date_fixed,
                         user_id, 
...
function get_kmntype_by_id ($db, $kmntype_id) {
  $query="SELECT * 
          FROM spr_kmn_type  
          WHERE id='".(int)$kmntype_id."'
          ";
  $res=mysql_query($query,$db);
  $type_kolvo=mysql_num_rows($res);
  $ar=mysql_fetch_assoc($res);
  if ($type_kolvo=='1'  ) {
...
// Функция для логирования действий с домами
          function loging_add_domrecord ($db, $p_user_id, $p_action_id, $p_dom_id, $p_descr, $p_xtra1, $p_xtra2) {
            $query="INSERT INTO logs 
                        (id,
                         date_fixed,
                         user_id, 
...
// Функция для логирования действий с участками
          function loging_add_uchrecord ($db, $p_user_id, $p_action_id, $p_uch_id, $p_descr, $p_xtra1, $p_xtra2) {
            $query="INSERT INTO logs 
                        (id,
                         date_fixed,
                         user_id, 
...
function get_uch_obyav_by_id ($db, $uch_id) {
  ...
  if ($uch_kolvo=='1'  ) {
    ...
    // + СТРОЕНИЯ
    if ($ar['uch_stroen']=="1") { $ld.="со строениями, "; };
...
function get_dom4sale_adr_by_id ($db, $dom_id) {
  ...
  if ($kv_kolvo=='1'  ) {
    ...
    // Нет такого объявления
    return "[Неизвестное объявление о продаже дома/коттеджа, id=".(int)$dom_id."]";
  };
};  
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
function make_thumb($src,$dest,$new_h)
{
	...
	/* find the "new height" of this thumbnail, relative to the desired width  */
	...
	// imagecopyresized($virtual_image,$source_image,0,0,0,0, $new_w,$new_h,$w,$h); // БЕЗ СГЛАЖИВАНИЯ
	
	imagecopyresampled($virtual_image,$source_image,0,0,0,0, $new_w,$new_h,$w,$h); // СО СГЛАЖИВАНИЕМ
...
// Функция возвращает ИМЯ ФАЙЛА СТРАНИЦЫ для просмотра подробностей - по номеру типа объекта 
...
function check_object_avtor($db, $user_id, $obj_type, $obj_id) {
  ...
  if ( $table_name=get_table_by_objtype ($db, $obj_type) ){
    $query="SELECT * 
            FROM ".$table_name." 
            WHERE id='".(int)$obj_id."' AND 
                  status='0' AND 
                  usr_id='".(int)$user_id."'
            ";
    $res=mysql_query($query,$db);
    $zapis_kolvo=mysql_num_rows($res);
    //$ar=mysql_fetch_assoc($res);
...
// Функция для логирования действий с квартирами
          function unilog_add_record ($db, $p_user_id, $p_action_id, $p_obj_type, $p_obj_id, $p_descr, $p_xtr1, $p_xtr2) {
            $query="INSERT INTO lib_unilog 
                        (id,
                         date_fixed,
                         user_id, 
                         action_id, 
