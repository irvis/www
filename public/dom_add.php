...
<html>
<head>
  <title>Подать объявление о продаже коттеджа / дома в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
if ($glogged==1) { // Пользователь авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    // Если ошибок нет
      ...
      $query="INSERT INTO sell_dom SET ";
      //$query.="status='0',";
      //$query.="checked_fl='0',";
      ...
      // Записываем в журнал
      loging_add_domrecord ($db, $_SESSION['id'], "301", $last_id, get_dom_obyav_by_id($db,$last_id), get_dom_descr_by_id ($db, $last_id), get_dom_conta_by_id ($db, $last_id) );
      
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление добавлено!</b> Посмотреть опубликованное объявление можно <a href=\"./prodam_dom.php?id=".$last_id."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
...
?>    
       ...
       <h2>Продать коттедж / дом в Великом Новгороде</h2>
       <div class="clear"></div>
       
       <form name="myform" action="dom_add.php" method="post">
       <!-- ТАБЛИЦА -->
       ...
       <tr>
         ...
         <td>
           <select name="dom_type">
             <?php printselectsortdef($db, 'spr_dom_type', 'sort', 'ASC', $_POST['dom_type']);?>
           </select>
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <select name="dom_material_id">
             <?php printselectdef($db, 'spr_zd_material', $_POST['dom_material_id']); ?>
           </select>
         </td>
       </tr>
       ...
       <tr>
         <td>Этажность * :</td>
         <td>
           <input name="dom_floors" type="edit" size="3" value="<?php if (isset($_POST['dom_floors'])) { echo $_POST['dom_floors']; };?>">
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="dom_pl_ob" size="4" type="edit" value="<?php if (isset($_POST['dom_pl_ob'])) { echo $_POST['dom_pl_ob']; };?>">
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="dom_pl_zh" size="4" type="edit" value="<?php if (isset($_POST['dom_pl_zh'])) { echo $_POST['dom_pl_zh']; };?>">
         </td>
       </tr>
       ...
       <tr>
         <td>Примечание:</td>
         <td>
           <textarea title="Дополнительная информация по дому (Например: сделан евроремонт, установлены стеклопакеты)" name="descr" cols="30" rows="4"><?php if (isset($_POST['descr'])) { echo $_POST['descr']; }; ?></textarea>
         </td>
       </tr>
