...
<html>
<head>
  <title>Подать объявление о продаже участка, дачи в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
if ($glogged==1) { // Пользователь авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    $errorlist="";
    // здесь вставляется проверка параметров (добавление дома)
    ...
    // Если ошибок нет
      ...
      $query="INSERT INTO sell_uch SET ";
      //$query.="status='0',";
      //$query.="checked_fl='0',";
      ...
      // Записываем в журнал
      loging_add_uchrecord ($db, $_SESSION['id'], "401", $last_id, get_uch_obyav_by_id($db,$last_id), get_uch_descr_by_id ($db, $last_id), get_uch_conta_by_id ($db, $last_id) );
      
      
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление добавлено!</b> Посмотреть опубликованное объявление можно <a href=\"./prodam_uchastok.php?id=".$last_id."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
...
?>    
       ...
       <h2>Продать участок, дачу в Великом Новгороде</h2>
       <div class="clear"></div>
       
       <form name="myform" action="uchastok_add.php" method="post">
       <!-- ТАБЛИЦА -->
       ...
       <tr>
         ...
         <td>
           <select name="uch_type">
             <?php printselectsortdef($db, 'spr_uch_type', 'sort', 'ASC', $_POST['uch_type']);?>
           </select>
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           ...
           <input name="uch_pl" type="edit" size="6" value="<?php if (isset($_POST['uch_pl'])) { echo $_POST['uch_pl']; };?>">
           
           <select name="ed_pl">
             <option value="1" <?php if ( (isset($_POST['ed_pl'])) && ($_POST['ed_pl']=='1') ) { echo "selected"; };?> >Соток</option>
             <option value="2" <?php if ( (isset($_POST['ed_pl'])) && ($_POST['ed_pl']=='2') ) { echo "selected"; };?>>Гектар</option>
           </select>
         </td>
       </tr>
       ...
       <tr>
         ...
         <td>
           <input name="uch_stroen" value="1" type="checkbox" <?php if (isset($_POST['uch_stroen'])) { echo "checked"; };?> > есть строения на участке (их необходимо описать в поле &laquo;Примечание&raquo;)
         </td>
       </tr>
       ...
       <tr>
         <td>Примечание:</td>
         <td>
           <textarea title="Дополнительная информация по участку (Например: участок разработан, построена баня, летний домик, есть электричество, сделан подъезд)" name="descr" cols="30" rows="4"><?php if (isset($_POST['descr'])) { echo $_POST['descr']; }; ?></textarea>
         </td>
       </tr>
