...
<html>
<head>
  <title>Редактирование объявления о продаже квартиры в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
if (!($glogged==1)) { // Пользователь не авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    if ($errorlist!="") {
      ...
      loging_add_kvrecord ($db, $_SESSION['id'], "103", (int)$_GET['id'], get_kv_obyav_by_id($db,(int)$_GET['id']), get_kv_descr_by_id ($db, (int)$_GET['id']), get_kv_conta_by_id ($db, (int)$_GET['id']) );
      ...
      unilog_add_record (
		 ...
		 "Редактирование (старое): ".get_kv_obyav_by_id($db,(int)$_GET['id']),   // Описание - генерация текста объявления
		 "Старое описание: ".get_kv_descr_by_id ($db, (int)$_GET['id']), // Допинфа1 - Описание
		 "Старые контакты: ".get_kv_conta_by_id ($db, (int)$_GET['id'])  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
      ...
      unilog_add_record (
		 ...
		 "11",  // Тип объекта - Квартира на продажу - 11
		 (int)$_GET['id'], // Номер объекта
		 "Редактирование (новое): ".get_kv_obyav_by_id($db,(int)$_GET['id']),   // Описание - генерация текста объявления
		 "Новое описание: ".get_kv_descr_by_id ($db, (int)$_GET['id']), // Допинфа1 - Описание
		 "Новые контакты: ".get_kv_conta_by_id ($db, (int)$_GET['id'])  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
      
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление успешно отредактировано!</b> Посмотреть отредактированное объявление можно <a href=\"./prodam_kvartiru.php?id=".intval($_GET['id'])."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
  ...
  if (($found) && ($showme)) {  ?>    
       <h2>Редактирование объявления о продаже квартиры</h2>
       <div class="clear"></div>
       
       <form name="myform" action="kvartira_edit.php?id=<?php echo intval($_GET['id']); ?>" method="post">
       <!-- ТАБЛИЦА -->
