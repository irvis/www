...
<html>
<head>
  <title>Редактирование объявления о сдаче в аренду комнаты в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
if (!($glogged==1)) { // Пользователь не авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    if ($errorlist!="") {
      ...
      unilog_add_record (
		 ...
		 "Редактирование (старое): ".get_kmn_rent_obyav_by_id($db,(int)$_GET['id']),   // Описание - генерация текста объявления
		 "Старое описание: ".get_kmn_rent_descr_by_id ($db, (int)$_GET['id']), // Допинфа1 - Описание
		 "Старое контакты: ".get_kmn_rent_conta_by_id ($db, (int)$_GET['id'])  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
      ...
      unilog_add_record (
		 ...
		 "22",  // Тип объекта - Комната в аренду - 22
		 (int)$_GET['id'], // Номер объекта
		 "Редактирование (новое): ".get_kmn_rent_obyav_by_id($db,(int)$_GET['id']),   // Описание - генерация текста объявления
		 "Новое описание: ".get_kmn_rent_descr_by_id ($db, (int)$_GET['id']), // Допинфа1 - Описание
		 "Новые контакты: ".get_kmn_rent_conta_by_id ($db, (int)$_GET['id'])  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
	  
      
      echo "<h2>Поздравляем!</h2>";
      echo "<b style=\"color:green\">Ваше объявление успешно отредактировано!</b> Посмотреть отредактированное объявление можно <a href=\"./sdam_komnatu.php?id=".(int)$_GET['id']."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
  ...
  if (($found) && ($showme)) {  ?>    
       <h2>Редактирование объявления о сдаче в аренду комнаты</h2>
       <div class="clear"></div>
       
       <form name="myform" action="arenda_komnat_edit.php?id=<?php echo intval($_GET['id']); ?>" method="post">
       <!-- ТАБЛИЦА -->
