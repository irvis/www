...
<html>
<head>
  <title>Подать объявление о продаже квартиры в Великом Новгороде - Проект &laquo;Новгородская квартира&raquo; (Недвижимость Великого Новгорода)</title>
...
if ($glogged==1) { // Пользователь авторизован
  ...
  // Если что-то передано через POST - проверка на вшивость!
    ...
    require "./_parts/check_kvartira.php";
    ...
    // Если ошибок нет
      ...
      $query="INSERT INTO sell_kva SET ";
      //$query.="status='0',";
      //$query.="checked_fl='0',";
      ...
      // Записываем в журнал
      loging_add_kvrecord ($db, $_SESSION['id'], "101", $last_id, get_kv_obyav_by_id($db,$last_id), get_kv_descr_by_id ($db, $last_id), get_kv_conta_by_id ($db, $last_id) );
      ...
      unilog_add_record (
		 ...
		 "Добавление объявления: ".get_kv_obyav_by_id($db,$last_id),   // Описание - генерация текста объявления
		 "Описание: ".get_kv_descr_by_id ($db, $last_id), // Допинфа1 - Описание
		 "Контакты: ".get_kv_conta_by_id ($db, $last_id)  // Допинфа2 - Контакты
	  );
	  // Записали в журнал
      ...
      echo "<b style=\"color:green\">Ваше объявление добавлено!</b> Посмотреть опубликованное объявление можно <a href=\"./prodam_kvartiru.php?id=".$last_id."\" class=\"link\">здесь</a>.<br>"; 
      unset($_POST);
      $showme=false;
...
?>    
       ...
       <h2>Продать квартиру в Великом Новгороде</h2>
       <div class="clear"></div>
       
       <form name="myform" action="kvartira_add.php" method="post">
       <!-- ТАБЛИЦА -->
